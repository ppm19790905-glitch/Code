...
function update(dt){updateFx(dt);state.tt=Math.max(0,state.tt-dt);if(state.mode==='title'){if(hit('z','enter',' '))reset();if(hit('l'))load()}else if(state.mode==='world'){move(dt);if(hit('z','enter',' ')){const n=nearest();if(n)act(n[3])}if(hit('p'))save();if(hit('l'))load()}else if(state.mode==='dialog'){if(hit('z','enter',' ')){state.dlg.i++;if(state.dlg.i>=state.dlg.lines.length){const d=state.dlg.done;state.dlg=null;state.mode='world';d?.()}}}else if(state.mode==='battle'){const b=state.battle;if(hit('arrowup','w'))b.sel=(b.sel+2)%3;if(hit('arrowdown','s'))b.sel=(b.sel+1)%3;if(hit('z','enter',' ')){if(b.sel===2){state.mode='world';state.battle=null}else{const dmg=b.sel===0?Math.max(4,18+Math.floor(Math.random()*8)-4):8+Math.floor(Math.random()*9);b.hp=Math.max(0,b.hp-dmg);b.log.push((b.sel===0?'物理':'食レポ')+' '+dmg+'ダメージ');impact('enemy',dmg,b.sel===0?C.gold:'#d58bc5');beep(b.sel===0?150:740);if(b.hp<=0){const after=b.after;state.mode='world';state.battle=null;state.gold+=20;after?.();save(false)}else{const hurt=Math.max(1,b.atk-7+Math.floor(Math.random()*5));state.hp=Math.max(1,state.hp-hurt);b.log.push(b.name+'の攻撃 '+hurt+'ダメージ');impact('player',hurt,'#e27a6d')}}}}else if(state.mode==='ending'&&hit('z','enter',' '))state.mode='title';P.clear()}
