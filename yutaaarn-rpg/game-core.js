...
const K=new Set(),P=new Set();let sound=false,audio=null,dir='down';
const C={ink:'#17161c',paper:'#f3ead2',gold:'#d0ad58',road:'#67635d',grass:'#667f58',grass2:'#758e63',stone:'#756d61',roof:'#39353a',wood:'#76543d',water:'#537889',red:'#94504a',blue:'#405d72',ui:'#202129'};
...
addEventListener('keydown',e=>{const k=e.key.toLowerCase();K.add(k);P.add(k);if(['arrowup','arrowdown','arrowleft','arrowright',' ','enter'].includes(k))e.preventDefault()});
...
function load(){try{const d=JSON.parse(localStorage.getItem(SAVE));if(!d)return toast('セーブなし');Object.assign(state,{area:d.area??0,stage:d.stage??0,hp:d.hp??72,gold:d.gold??120,done:d.done??{},mode:'world'});px=d.px??320;py=d.py??220;dir=d.dir??'down';toast('ロードしました')}catch{toast('ロード失敗')}}
function reset(){Object.assign(state,{mode:'world',area:0,stage:0,hp:72,gold:120,done:{},dlg:null,battle:null});px=320;py=220;dir='down'}
